...
struct TensorDesc : public Dim {
  ...
  TensorDesc(int n, int c, int h, int w) : Dim(n, c, h, w) {
    CHECK_MIO(miopenCreateTensorDescriptor(&desc),"miopenCreateTensorDescriptor");
    CHECK_MIO(miopenSet4dTensorDescriptor(desc, miopenFloat, n, c, h, w),"miopenCreateTensorDescriptor");
  }
  ...
  void update_get() {
    ...
    CHECK_MIO(miopenGet4dTensorDescriptor(desc, &dt, &n, &c, &h, &w, &ns, &cs,
                                          &hs, &ws),"miopenCreateTensorDescriptor");
    assert(dt == miopenFloat);
  }

  void free() {
    if (!(n == 0 && c == 0 && h == 0 && w == 0)) {
      CHECK_MIO(miopenDestroyTensorDescriptor(desc),"miopenCreateTensorDescriptor");
    }
  }
...
struct Tensor : public TensorDesc {
  ...
  */
  ...
  void print_data() {
    std::vector<float> hostTensor = toHost();
    assert(hostTensor.size() == (size_t)n * c *h *w);
    std::cout << "Tensor of size " << *this << ":" << std::endl << "[";
    for (int i = 0; i < n; ++i) {
      std::cout << "[";
      for (int j = 0; j < c; ++j) {
        ...
        for (int m = 0; m < h; ++m) {
          std::cout << "[";
          for (int e = 0; e < w; ++e) {
            std::cout << hostTensor[i*c*h*w+j*h*w+m*w+e];
            if(e+1==w)
              std::cout << "] ";
            else
              std::cout << ", ";
  ...
  void initData_fromCPU(float *cpu_data,size_t data_lens){
    hipMemcpyHtoD(data, cpu_data, data_lens*sizeof(float));
  }
  Tensor(TensorDesc &&d)
...
std::string GetConvolutionAlgorithmName(miopenConvAlgorithm_t algorithm) {
    switch (algorithm) {
...
void rand_float(float *inputs, size_t len, float min ,float max){
    static std::random_device rd;
    static std::mt19937 mt(42);
    static std::uniform_real_distribution<float> dist(min, max);

    for(size_t i=0;i<len;i++){inputs[i] = dist(mt);}
}
