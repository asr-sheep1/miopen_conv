...
#define CHECK_MIO(cmd,strs)                                                         \
  {                                                                            \
    miopenStatus_t miostat = cmd;                                            \
    std::string logging = strs;                                             \
...
struct Devices {
  ...
  static void init_devices() {
    ...
    // }
    devs[0].hip_id = 0;
    CHECK_HIP(hipGetDeviceProperties(&devs[0].hip_props, 0 /*deviceID*/));
    devs[0].init_sys_paths();
    devs[0].print_info();
...
private:
  ...
  static miopenHandle_t init_mio() {
    ...
    CHECK_MIO(miopenCreateWithStream(&h, q),"[miopenCreateWithStream]");
    return h;
  }
